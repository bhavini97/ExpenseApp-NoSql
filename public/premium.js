...
document.getElementById('pageLimit').addEventListener('change', () => {
    ...
    async function fetchExpenses() {
        try {
            const response = await fetch(`/premium/expenses?page=${currentPage}&limit=${limit}`, {
    ...
    try{
        const response = await fetch(`/expense/download`,{
            headers : {Authorization: `Bearer ${localStorage.getItem("token")}` },
        })
    ...
    async function fetchDownloadedFiles() {
        try {
            const response = await fetch("/premium/downloads", {
                headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
            });
            const data = await response.json();
